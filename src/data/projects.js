export const projects = [
  {
    ...
    blogContent: [
      "Pongs Shipping came to me with a clear problem but no clear system. The business was already running. Customers were calling in, packages were moving, and money was being made, but the infrastructure holding it all together was a patchwork of spreadsheets and processes that existed mostly in people's heads. Some things were tracked digitally. Others were written on paper. A lot was simply remembered by the right person at the right time. I was brought in to replace that with something that could actually scale.",
      "The challenge was that Pongs is not a simple business to model. It operates on two sides simultaneously, each with completely different needs. Customers needed a way to get dedicated Florida shipping addresses in PSC format, submit package pre-alerts with receipt uploads, and track their shipments through six status stages without ever picking up the phone. Meanwhile, the staff needed tools for package intake, transfer list generation with package check-off, revenue tracking in Jamaican dollars, and daily performance visibility, all within a single platform.",
      "The hardest part of the build was the six-tier role system. On paper, six roles sounds structured. In practice, the way a freight forwarding business actually operates means the boundaries between who does what are not always clean. A Handler and a Transfer Agent touch some of the same data but for completely different reasons. A Front Desk worker needs visibility into things they cannot act on. An Admin can do almost everything, but not quite as much as a Super Admin. Getting those lines right required a lot of back-and-forth with the client to understand the real-world workflow before I could translate it into code. I built exactly what the business needed, nothing more.",
      "On the technical side, the platform is built with React 19 on the frontend and a Node.js 20 and Express 5 backend backed by PostgreSQL. Receipt uploads and file storage run through AWS S3. SendGrid handles automated email notifications at every shipment stage, so customers stay informed without any manual effort from staff. The whole application is containerized with Docker and deployed on Railway.",
      "Since launch, operational efficiency at Pongs improved by 80%. The paperwork is gone. Every status change triggers an automated notification. Staff have a full audit trail with daily performance data. The business that was running on memory and spreadsheets now has a platform built for its actual scale. I am proud of what got built here, and equally proud that I took the time to understand how the business actually worked before I started writing a single line of code."
    ]
  },
  {
    id: "jewels-and-time",
